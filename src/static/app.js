document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities(newParticipantEmail = null) {
    try {
        ...
        if (details.participants && details.participants.length > 0) {
          participantsList = '<ul class="participants-list">' +
            details.participants.map(email => {
              const isNew = email === newParticipantEmail;
              const newBadge = isNew ? '<span class="new-badge">New!</span>' : '';
              return `
              <li class="${isNew ? 'new-participant' : ''}">
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Refresh the activities list to show the new participant with highlight
